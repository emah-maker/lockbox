...
- Firmware side: `firmware/lib/lock_config.py` (UUIDs), `firmware/lib/lock_ble.py` / `lock_controller.py` (payload handling)
...
1. Change `protocol.ts` and the matching `firmware/lib/lock_*.py` file in the same change.
