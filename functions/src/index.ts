...
//                          already do locally.
//
// collectPushReceipts is DEFINED in pushReceipts.ts and re-exported below.
// Firebase deploys by exported name from this entry point, so where the
// definition lives is invisible to deployment -- but the re-export is not
// optional: drop it and that job silently stops being deployed at all, while
...
// The receipt half of Expo delivery runs on its own schedule and with its own
// rules about what may cost a token its document -- see pushReceipts.ts.
import { recordTickets } from './pushReceipts';
export { collectPushReceipts } from './pushReceipts';
