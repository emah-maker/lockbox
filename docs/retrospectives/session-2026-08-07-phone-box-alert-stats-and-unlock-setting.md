...
1. **None proposed as durable rule changes from this single run**, per this job's own guardrail against promoting a per-incident coaching moment directly into a rule; the jest-mock-verification gap was captured as a standalone coaching moment (`fraim/personalized-employee/learnings/raw/2026-08-07T17-30-00-jest-mock-file-needs-registration.md`) for `sleep-on-learnings` to evaluate in aggregate.
